//! Vendored from `candle-transformers` 0.11.0, `models/quantized_llama.rs`.
//! Copyright the candle authors, Apache-2.0 / MIT.
//!
//! Verbatim in this commit so the next one's diff is the whole of what Lattice
//! changes. We fork rather than depend because a shard must load only the
//! tensors for its own layer range — `ModelWeights::from_gguf` reads every
//! layer, which would defeat the memory pooling this project exists for.
